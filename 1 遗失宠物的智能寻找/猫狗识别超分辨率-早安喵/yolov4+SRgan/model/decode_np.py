...
class Decode(object):
    ...
    def detect_image(self, image, draw_image):
        ...
        for box, score, cl in zip(boxes, scores, classes):
            if 15 == classes[i] or 16 == classes[i]:
                boxes_catdog.append(boxes[i])
                scores_catdog.append(scores[i])
                classes_catdog.append((classes[i]))
            i += 1

        if boxes is not None and draw_image:
            self.draw(image, boxes_catdog, scores_catdog, classes_catdog)
        return image, boxes_catdog, scores_catdog, classes_catdog
    ...
    def draw(self, image, boxes, scores, classes):
        ...
        for box, score, cl in zip(boxes, scores, classes):
            ...
            cv2.putText(image, bbox_mess, (left, top - 5), cv2.FONT_HERSHEY_SIMPLEX,
