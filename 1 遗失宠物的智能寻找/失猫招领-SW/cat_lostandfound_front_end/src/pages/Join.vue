<template>
  <div class="join">
    <div class="join-header">
      ...
      <h1 class="join-header-title">失猫招领</h1>
    </div>
    <div class="join-form">
    ...
    methods: {
      async onSubmit(values) {
        try {
          ...
          const response = await this.$request.join(values)
          ...
          const timer = setInterval(() => {
            second--
            if (second) {
              toast.message = `你的账号已经创建, ${second} 秒后将回到登录页`
            } else {
              clearInterval(timer)
              this.$toast.clear()
            }
